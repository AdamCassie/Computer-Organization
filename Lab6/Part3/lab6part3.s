/* Program to display COUNT on LEDR*/
...
/* Enable IRQ interrupts in the ARM processor */
				...
				MOV		R0, #0b10010		// R0 holds IRQ mode bits
				MSR		CPSR, R0			// Change to IRQ mode
				LDR		SP, =0x20000		// Set SP for IRQ mode
				MOV		R0, #0b10011		// R0 holds SVC mode bits
				MSR		CPSR, R0			// Change to SVC mode
											// Set Interrupt Mask bit to zero to enable interrupts
...
CONFIG_TIMER:
				PUSH	{R0, R1}
				LDR		R0, =0xFF202000		// R0 holds base address for Interval Timer Registers
				LDR 	R1, =0x7840
				STR 	R1, [R0, #0x8]		// Set lower bits for countdown value (#25000000)
				LDR 	R1, =0x17D
				STR 	R1, [R0,#0xC]		// Set upper bits for countdown value (#25000000)
				MOV 	R1, #0b0101			// R1 holds bits to control timer
				STR		R1, [R0, #0x4]		// Set Interval timer control register bits
				POP		{R0, R1}
				BX 		LR
...
CONFIG_KEYS:
				PUSH	{R0, R1}
				LDR		R0, =0xFF200050		// R0 holds address for KEYS
				MOV		R1, #0b1111			// R1 holds bits to enable IRQ generation for KEY3-0
				STR		R1, [R0, #0x8]		// Store bits in Interrupt Mask register for KEYs
				POP		{R0, R1}
				BX 		LR
...
KEY_ISR:		PUSH	{R0-R2}
				LDR		R0, =0xFF200000		// R0 holds address for LEDR
				LDR		R1, [R0, #0x5C]		// Read Edgecapture register for KEYs
				CMP		R1, #0b0001			// Check if KEY0 pressed
				BEQ		TOGGLE
				CMP		R1, #0b0010			// Check if KEY1 pressed
				BEQ		DOUBLE
				CMP		R1, #0b0100			// Check if KEY2 pressed
				BEQ		HALF
...
TOGGLE:			STR		R1, [R0, #0x5C]		// Reset Edgecapture register for KEYs
				LDR		R0, RUN				// R0 holds the value of RUN
				CMP		R0, #0
				BEQ		INCREMENT		
				
HOLD:			
				MOV		R1, #0			// R1 holds value to change RUN from 1 to 0
				STR		R1, RUN			// Change value of RUN to 0
				POP		{R0-R2}
				B		EXIT_IRQ
				

INCREMENT:		
				MOV		R1, #1			// R1 holds value to change RUN from 0 to 1
				STR		R1, RUN			// Change value of RUN to 1
				POP		{R0-R2}
				B		EXIT_IRQ
				
				
DOUBLE:			STR		R1, [R0, #0x5C]		// Reset Edgecapture register for KEYs
				LDR		R0, =0xFF202000		// R0 holds base address for Interval Timer Registers
				MOV		R1, #0b1000			// R1 holds bits to stop timer
				STR		R1, [R0,#0x4]		// Stop Timer
				LDR		R1, [R0, #0X8]		// R1 holds lower bits of countdown number
				LDR		R2, [R0, #0XC]		// R2 holds upper bits of countdown number
				LSL		R2, #16				// Shift msb's to left
				ADD		R1, R2				// R1 holds value of countdown number
				LSR		R1, #1				// Half start time to double the rate of the timer
				STR		R1, [R0, #0X8]		// Store lower bits of new countdown number
				LSR		R1, #16				// Shift upper bits right
				STR		R1, [R0, #0XC]		// Store upper bits of new countdown number
				MOV		R1, #0b0101			// R1 holds control bits to restart timer		
				STR		R1, [R0, #0x4]		// Restart timer
				POP		{R0-R2}
				B		EXIT_IRQ
				

HALF:			STR		R1, [R0, #0x5C]		// Reset Edgecapture register for KEYs
				LDR		R0, =0xFF202000		// R0 holds base address for Interval Timer Registers
				MOV		R1, #0b1000			// R1 holds bits to stop timer
				STR		R1, [R0,#0x4]		// Stop Timer
				LDR		R1, [R0, #0X8]		// R1 holds lower bits of countdown number
				...
				LSL		R2, #16				// Shift msb's to left
				ADD		R1, R2				// R1 holds value of countdown number
				LSL		R1, #1				// Double start time to half the rate of the timer
				STR		R1, [R0, #0X8]		// Store lower bits of new countdown number
				LSR		R1, #16				// Shift upper bits right
				STR		R1, [R0, #0XC]		// Store lower bits of new countdown number
				MOV		R1, #0b0101			// R1 holds control bits to restart timer		
				STR		R1, [R0, #0x4]		// Restart timer
				POP		{R0-R2}
				B		EXIT_IRQ
...
TIMER_ISR:		PUSH	{R0, R1}
				LDR		R0, COUNT			// R0 holds the value of COUNT
				LDR		R1, RUN				// R1 holds the value of RUN
				ADD		R0, R1				// Increment value of COUNT
				STR		R0, COUNT			// Set incremented value of COUNT
				LDR		R0, =0xFF202000		// R0 holds base address for Interval Timer Registers
				MOV		R1,	#0				// R1 holds value to reset timeout signal
				STR		R1, [R0]			// Reset timeout signal in Interval Timer status register
				MOV		R1, #0b0101			// R1 holds control bits to restart timer		
				STR		R1, [R0, #0x4]		// Restart timer
				POP		{R0, R1}
				B		EXIT_IRQ
...
/*address_map_arm.s code below*/

/* The following provides address values that exist in the system */
...
/*interrupt_ID.s code below*/

/* The following provides interrupt IDs */
