/* Program to display 3-0 on HEX3-0 when corresponding KEY is pressed*/
...
KEY_ISR:		PUSH	{R0-R3, LR}
				...
				BEQ		TWO					
				CMP		R1, #0b1000			// Check if KEY3 pressed
				BEQ		THREE
				
				
ZERO:			LDR		R2, =ZERO_PATTERN	// R2 holds the address of the pattern for zero
				LDRB	R2, [R2]			// R2 holds the zero pattern
				B		CHECK_HEX


ONE:			LDR		R2, =ONE_PATTERN	// R2 holds the address of the pattern for one
				LDRB	R2, [R2]			// R2 holds the one pattern
				LSL		R2, #8				// Shift to display on HEX1
				B		CHECK_HEX
				
				
TWO:			LDR		R2, =TWO_PATTERN	// R2 holds the address of the pattern for two
				LDRB	R2, [R2]			// R2 holds the two pattern
				LSL		R2, #16				// Shift to display on HEX2
				B		CHECK_HEX
				
				
THREE:			LDR		R2, =THREE_PATTERN	// R2 holds the address of the pattern for two
				LDRB	R2, [R2]			// R2 holds the three pattern
				LSL		R2, #24				// Shift to display on HEX3
				B		CHECK_HEX
				

CHECK_HEX:		LDR		R3, [R0, #0x20]		// Read value stored on HEX3-0
				CMP		R2, R3				// Check if pattern already stored (same KEY pressed successively)	
				BNE		DISPLAY				// Write pattern to HEX if it is not presently displayed

				
CLEAR:			MOV		R2, #0				// R2 holds bits to clear display
				
DISPLAY:		STR		R2, [R0, #0x20]		// Write pattern onto HEX
				STR		R1, [R0, #0x5C]		// Write to Edgecapture register for KEYs to clear interrupt
				POP		{R0-R3, PC}
...
ZERO_PATTERN:	.byte	0b00111111
ONE_PATTERN:	.byte	0b00000110
TWO_PATTERN:	.byte	0b01011011
THREE_PATTERN:	.byte	0b01001111
